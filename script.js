...
// additionalFee (Additional Fee, combined-mode only — see Additional Mode below).
let items = [
  { date: "", desc: "Math: Functions", note: "", price: 100000, additionalFee: 0 },
];
let qrisDataUrl = null;

// Additional Mode (Titipan) — OFF by default. A completely separate table &
// form, does NOT touch/replace the main Session Details table above.
...
function renderPreview() {
  ...
  // Additional Mode table — only filled when the toggle is on. Main table
...
// ============ TAB 2: Kalkulator Titipan (independent feature) ============
...
function getPageBreakpoints(sheetEl, scale) {
  ...
  // Row-level breaks inside both tables (main + additional), so a page
  // break never lands mid-row.
...
async function captureInvoiceCanvas() {
  ...
  const cloneScrollWraps = clone.querySelectorAll(".table-scroll");
  cloneScrollWraps.forEach((w) => { w.style.overflow = "visible"; });
  ...
  // Additional Mode table: if it's hidden in the live page (toggle off),
  // it must also stay hidden in the exported PDF/PNG.
  ...
  try {
    ...
    // so measuring the wrong one would cut pages in the wrong place.
